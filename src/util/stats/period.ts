const PERIOD_REGEX = /^(\d+)([dhmwy])$/

const UNITS = ['h', 'd', 'w', 'm', 'y']

export const DEFAULT_PERIOD_SUGGESTIONS = [
  '1h',
  '24h',
  '7d',
  '30d',
...
export function parsePeriod(period: string): Date | null {
  ...
  if (!match)
    throw new Error(
      `Invalid period: "${period}". Use formats like 24h, 7d, 2w, 6m, 1y, or all.`,
    )
  const [, num, unit] = match
  const n = Number.parseInt(num, 10)
