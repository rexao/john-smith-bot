...
async function handlePeriodSubcommand(
  ...
  try {
    ...
    await i.reply({ content: (error as Error).message, ephemeral: true })
    return
  }
  ...
  if (type === 'chatters') {
    ...
    const components = buildPaginationComponents(
      'chatters',
      guildId,
      periodInput,
      1,
...
export default {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        .setName('scan')
        .setDescription('Scan server history to populate stats.'),
    )
    .toJSON(),
  ...
  async execute(interaction: CommandInteraction) {
    ...
    if (sub === 'scan') {
      await i.deferReply()
      const { events, channelCount, scannedIdsByChannel } = await scanGuild(
        i.guild,
      )
      const inserted = bulkInsert(events)
      const deleted = removeStaleMessages(scannedIdsByChannel)
      await i.followUp(
        `Scan complete. ${channelCount} channels scanned, ${events.length} messages found, ${inserted} new messages inserted, ${deleted} stale messages removed.`,
      )
    }
  },
} satisfies Command
