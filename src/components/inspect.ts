...
export type ServerPage =
  'bannerBackground' | 'icon' | 'inviteBackground' | 'main'
export type UserPage = 'avatar' | 'main' | 'profileBanner' | 'serverAvatar'

export function buildServerEmbeds(
  guild: Guild,
): Record<ServerPage, EmbedBuilder> {
  const now = new Date()
  ...
  if (guild.icon) {
    icon
      .setImage(guild.iconURL({ size: 4_096 }))
      .addFields({ name: 'Icon Hash', value: `\`${guild.icon}\`` })
  } else {
...
export function buildServerComponents(
  ...
  if (page === 'main') {
    row.addComponents(
      new ButtonBuilder()
        .setCustomId('inspect:server:icon')
        .setLabel('Icon')
    ...
    row.addComponents(
      new ButtonBuilder()
        .setCustomId('inspect:server:main')
        .setLabel('< Back')
...
export async function buildUserEmbeds(
  member: GuildMember,
  client: Client,
): Promise<Record<UserPage, EmbedBuilder>> {
  const now = new Date()
  const { user } = member
  ...
  if (user.avatar) {
    avatar
      .setImage(user.avatarURL({ size: 4_096 }))
      .addFields({ name: 'Avatar Hash', value: `\`${user.avatar}\`` })
  } else {
    avatar
      .setDescription(`${user.toString()} has no avatar.`)
  ...
  if (member.avatar) {
    ...
    serverAvatar
      .setDescription(`${user.toString()} has no server avatar.`)
      .setImage(displayAvatarURL)
  }
...
export function buildUserComponents(
  userId: UserId,
  page: UserPage,
  inGuild: boolean,
): ActionRowBuilder<ButtonBuilder> {
